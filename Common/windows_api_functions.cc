...
namespace APIFunctions {
  ...
  rj2xcl::Result<std::string, FileError> APIFunctions::FileContents(const std::string &path) {
    ...
    if (!file_handle || file_handle == INVALID_HANDLE_VALUE) {
      return rj2xcl::Result<std::string, FileError>::Failure(FileError::FileNotFound);
    }
    else {
      ...
      while (true) {
        if(ReadFile(file_handle, buffer, buffer_size, &bytes_read, 0)){
          ...
          delete[] buffer;
          return rj2xcl::Result<std::string, FileError>::Failure(FileError::FileReadError);
        }
      }
      ...
      return rj2xcl::Result<std::string, FileError>::Success(contents);
    }
  }
