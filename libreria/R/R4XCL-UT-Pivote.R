...
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++
# RECODIFICA VALORES EN UN RANGO
# Sustituye valores originales por nuevos valores
#+++++++++++++++++++++++++++++++++++++++++++++++++++++++

DB_Recodificar <- function(SetDatosX, ValoresOriginales, ValoresNuevos)
{
  # SetDatosX:         Rango a modificar (con o sin encabezado)
  # ValoresOriginales: Rango con los valores a buscar
  # ValoresNuevos:     Rango con los valores de reemplazo (mismo orden)

  datos   <- as.matrix(SetDatosX)
  viejos  <- as.character(as.matrix(ValoresOriginales))
  nuevos  <- as.character(as.matrix(ValoresNuevos))

  # Validar que ambos vectores tengan la misma longitud
  if (length(viejos) != length(nuevos)) {
    return("ERROR: Los rangos de valores originales y nuevos deben tener la misma cantidad de elementos")
  }

  # Aplicar recodificación
  resultado <- datos
  for (i in seq_along(viejos)) {
    resultado[datos == viejos[i]] <- nuevos[i]
  }

  return(resultado)
}

attr(DB_Recodificar, "description") <- "Recodifica valores en un rango sustituyendo valores originales por nuevos"
attr(DB_Recodificar, "category")    <- "Data Management"
