...
)
{
  ...
  if (TipoOutput <= 0){
    ...
    OutPut <- Extraer_outputs(Modelo, "MR_Lineal")
    # Simplificar para Excel: solo Seccion, Parametro, Metrica, Valor
    if (is.data.frame(OutPut) && nrow(OutPut) > 0) {
      OutPut <- OutPut[, c("Seccion", "Parametro", "Metrica", "Valor")]
    }
