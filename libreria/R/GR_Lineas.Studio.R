# ===============================================================================
# NEVEN Data Lab — GR_Lineas.Studio.R
# Gráfico de Líneas con soporte para múltiples series (rol Color)
# ===============================================================================
# Parámetros:
#   data_X         data.frame — rol X (numérico o texto, requerido)
#   data_Y         data.frame — rol Y (numérico, requerido)
#   data_Color     data.frame — rol Color/Series (texto, opcional)
#   Titulo         character  — título del gráfico (default "")
...
GR_Lineas.Studio <- function(data_X,
                               data_Y,
                               data_Color     = NULL,
                               Titulo         = "",
                               MostrarLeyenda = TRUE,
  ...
  # Título opcional
  if (!is.null(Titulo) && nchar(trimws(Titulo)) > 0) {
    layout$title <- list(
      text = Titulo,
  ...
  if (!is.null(data_Color) && is.data.frame(data_Color) && nrow(data_Color) == nrow(data_X)) {
    ...
    for (i in seq_along(grupos)) {
      ...
      # Ordenar por x_vec dentro del grupo para evitar líneas que cruzan
    ...
    # ── Modo serie única ───────────────────────────────────────────────────
    # Ordenar datos por x_vec antes de graficar para evitar líneas que cruzan
    ord      <- order(x_vec)
    x_sorted <- x_vec[ord]
    y_sorted <- y_vec[ord]
  ...
  # ── 9. Codificación base64 y armado del slot HTML ─────────────────────────
  ...
  # ── 10. Retorno con r_object_to_slots ─────────────────────────────────────
