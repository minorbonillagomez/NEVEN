...
R4XCL_INT_LISTA_DATASETS = function(psPkg) 
{
  ...
  for(i in 1:nV ) 
    {
      x = class(get(v[i], envir = asNamespace(psPkg)))
                          ...
                          )
{
  ...
  if (Categorica == 0) {
    muestra <- SetDatosX[-1, , drop = FALSE]
    tiene_texto <- any(suppressWarnings(is.na(as.numeric(as.character(muestra)))) & !is.na(muestra))
    if (tiene_texto) Categorica <- 2
  }

  if (Categorica >= 1){
        
    # Ruta texto/factores (1=diálogo manual, 2=auto silencioso)
                                 ...
                                 )
{
  ...
  # Si el usuario requiere filtrar datos, proceder a conformar vector filtro  
    ...
    if (!is.null(Filtro))
       {Filtro = as.numeric(as.matrix(unlist(Filtro[-1,])))} 
    else 
       {Filtro=rep(0,pNObs)}
  ...
  #-------------------------->>>
    ...
    if (!is.null(SetDatosY)){
                            ...
                            DTY.F1 = SetDatosY[-1,]
                            DTY.F  = as.numeric(unlist(DTY.F1))
                            }
    ...
    } else if(Categorica==2){
                     # Auto-detección silenciosa por columna (sin diálogo)
                     DTX.F = data.frame(DTX.F, stringsAsFactors = FALSE)
                     for (col in NombreVariablesX) {
                       vals <- as.character(DTX.F[, col])
                       nums <- suppressWarnings(as.numeric(vals))
                       if (all(!is.na(nums) | is.na(vals))) {
                         DTX.F[, col] <- nums
                       } else {
                         DTX.F[, col] <- as.factor(vals)
                       }
                     }
    }
...
#       cat("Error: No se encontrÃ³ el paquete", nombre_archivo, "en el directorio especificado.\n")
#       return(FALSE)  # Detener la instalaciÃ³n si falta un paquete
#     }
#     
#     # Instalar el paquete
#     tryCatch({
...
#                        )
#       cat("El paquete", nombre_paquete, "se instalÃ³ correctamente.\n")
#       
#     }, error = function(e) {
#       cat("Error al instalar el paquete", nombre_paquete, ":", e$message, "\n")
#       return(FALSE)  # Detener la instalaciÃ³n si hay un error
#     })
#     
#     # Cargar el paquete
#     tryCatch({
#       library(nombre_paquete)
#       cat("El paquete", nombre_paquete, "se cargÃ³ correctamente.\n")
#       
#     }, error = function(e) {
