...
# Note (March 2018):
# 
# These are older functions built when we first wrote RJ2XCL and 
# before we added the scripting (COM) interface. 
# 
# These functions will continue to work in RJ2XCL 2, as long as you 
# source this file (or put it in the functions folder).
# 
# However we recommend that you use the scripting interface, as 
# we are focusing on that one going forward. That one is more 
# flexible and has more features, so it should be better going
# forward.
#
#====================================================================
...
# RJ2XCL.Excel, and then attach that namespace.  we do that so they're
# available in the console but don't get added as Excel functions.
#

if( any(search() == "RJ2XCL.Excel")){
	suppressMessages(detach( RJ2XCL.Excel ));
}

RJ2XCL.Excel <- new.env();
with( RJ2XCL.Excel, {
...
{
	ref <- RJ2XCL$.Excel( 0x4004, list( sheetName ));
...
Normalize.Formula <- function( formula.or.value ){
	if( typeof( formula.or.value ) == "character" 
		...
		formula.or.value <- RJ2XCL$.Excel( 241, list( formula.or.value, TRUE, FALSE, 1, coffset));
	}
		...
		if( !is.null( n.row )){
			...
			for( rx in seq( 0, nrow(ref) - 1 )){
				...
				for( cx in seq( 0, ncol(ref) - 1 )){
					target@C1 <- ref@C1 + cx;
					RJ2XCL$.Excel( 0x8000 + 96, list( Normalize.Formula(formula.or.value[rx+1,cx+1]), target ));
				}
			}
		}
		...
		# case 3: list/vector
			...
			if( column.first ){
				for( cx in seq( 0, ncol(ref) - 1 )){
					...
					for( rx in seq( 0, nrow(ref) - 1 )){
						target@R1 <- ref@R1 + rx;
						RJ2XCL$.Excel( 0x8000 + 96, list( Normalize.Formula(formula.or.value[index]), target ));
						index <- index + 1;
					}
				}
			}
...
Set.Cell <- function( ref, formula.or.value ) {
	RJ2XCL$.Excel( 0x8000 + 97, list( Normalize.Formula( formula.or.value ), Ensure.Ref( ref ) ));
}
...
Set.Array <- function( ref, formula.or.value ){
	RJ2XCL$.Excel( 0x8000 + 98, list( Normalize.Formula( formula.or.value ), Ensure.Ref( ref ) ));
}
...
Select.Range <- function( ref ) {
	ref <- Ensure.Ref( ref );
	RJ2XCL$.Excel( 0x8000 + 109, list( ref ));
}
...
Copy.Range <- function( ref ){
	ref <- Ensure.Ref( ref );
	RJ2XCL$.Excel( 0x8000 + 50, list( ref ))
}
...
Paste.Special <- function(){
	RJ2XCL$.Excel( 0x8000 + 53, list( 3, 1, FALSE, FALSE ));
}
...
Set.Echo <- function( new.state ){
	# RJ2XCL$.Excel( 0x8000 + 141, list( new.state ));
	RJ2XCL$.Excel( 87, list( new.state ));
}
	...
	if( len == 1 ){
		...
		if( !is.null( v.row )){
			...
			if( ref.row > v.row || ref.col > v.col ){
				...
				RJ2XCL$.Excel( 0x8000 + 97, list( Normalize.Formula( default.value ), ref ));
		...
		RJ2XCL$.Excel( 0x8000 + 98, list( Normalize.Formula( formula.or.value ), ref ));
...
Format.Range <- function( ref, number.format ){
	ref <- Ensure.Ref( ref );
	Select.Range( ref );
	RJ2XCL$.Excel( 0x8000 + 42, list( number.format ))
}
...
Run.Macro <- function( name ){
	RJ2XCL$.Excel( 0x8000 + 17, list(name));
}
...
Define.Name <- function( name, ref ){
	...
	ref <- Ensure.Ref( ref );
	RJ2XCL$.Excel( 0x8000+61, list(name, ref));
...
Delete.Name <- function( name ){
	...
	RJ2XCL$.Excel( 0x8000 + 110, list( name ));
...
Activate.Sheet <- function( name ){
	...
	RJ2XCL$.Excel( 0x8000 + 291, list( name, name, TRUE ));
...
suppressMessages(attach( RJ2XCL.Excel ));
