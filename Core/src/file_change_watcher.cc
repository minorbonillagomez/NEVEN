...
void FileChangeWatcher::WatchDirectory(const std::string &directory) {
  ...
  // Path resolution handled by Windows API notification system
  ...
  SetEvent(update_watch_list_handle_.get());
...
void FileChangeWatcher::StartWatch() {
  if (running_) {
    RJ2XCL_LOG_ERR("ERROR: already watching");
    return;
  }
...
uint32_t FileChangeWatcher::InstanceStartThread() {
  ...
  while (running_) {
    ...
    ResetEvent(update_watch_list_handle_.get());
    for (auto entry : watched_directories_) {
    ...
    for (auto entry : directories) {
      ...
      if (!handle || handle == INVALID_HANDLE_VALUE) {
        RJ2XCL_LOG_WARN("WARNING: bad handle @ %s", entry.c_str());
      }
    ...
    handles.push_back(update_watch_list_handle_.get());
    ...
    while (looping && running_) {
      ...
      DWORD timeout = change_list.size() ? rj2xcl::Constants::kFileWatchDebounceMs : rj2xcl::Constants::kFileWatchNormalMs;
      ...
      if (wait_result == WAIT_OBJECT_0 + count) {
        ...
        RJ2XCL_LOG_INFO("** Directory list change, restart");
        looping = false;
        break;
      }
        ...
        RJ2XCL_LOG_DEBUG("(directory change: %s)", directories[index].c_str());
        bool found = false;
        ...
        RJ2XCL_LOG_ERR("Watch failed with error %d", err);
        break; // ?
      }
    }
