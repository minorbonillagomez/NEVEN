...
namespace RJ2XCLGraphics {
  ...
  void QuerySize(const std::string &name, RJ2XCLBuffers::CallResponse &response, LPDISPATCH application_dispatch) {
    ...
    if (shape) {
      ...
      if (ishape) {
        ...
        graphics->set_command(RJ2XCLBuffers::GraphicsUpdateCommand::query_size);
  ...
  void UpdateGraphics(const RJ2XCLBuffers::GraphicsUpdate &graphics, LPDISPATCH application_dispatch) {
