...
COMObjectMap::EnumValues COMObjectMap::MapEnum(std::string &name, CComPtr<ITypeInfo> type_info, TYPEATTR *type_attributes)
{
  ...
  for (UINT u = 0; u < type_attributes->cVars; u++)
  {
    ...
    if (SUCCEEDED(hresult))
    {
      ...
      {
        RJ2XCL_LOG_WARN("enum type not const/I4");
      }
      else {
        CComBSTR name;
...
void COMObjectMap::MapInterface(std::string &name, std::vector< MemberFunction > &members, CComPtr<ITypeInfo> typeinfo, TYPEATTR *type_attributes)
{
  ...
  for (UINT u = 0; u < type_attributes->cFuncs; u++)
  {
    ...
    if (SUCCEEDED(hresult))
    {
      // Mask hidden (0x01) and restricted (0x40) functions/interfaces.
      if ((com_function_descriptor->wFuncFlags & (FUNCFLAG_FHIDDEN | FUNCFLAG_FRESTRICTED)) == 0) {
        ...
        if (com_function_descriptor->cParams > 0) {
          if (name_count > 1) {
            ...
            // Standard RHS parameter for property put.
...
void COMObjectMap::InvokeCOMPropertyPut(const RJ2XCLBuffers::CompositeFunctionCall &callback, RJ2XCLBuffers::CallResponse &response) {
...
void COMObjectMap::InvokeCOMFunction(const RJ2XCLBuffers::CompositeFunctionCall &callback, RJ2XCLBuffers::CallResponse &response)
{
  if (callback.type() == RJ2XCLBuffers::CallType::put) {
  ...
  if (SUCCEEDED(hresult) && type_info_pointer)
  {
    ...
    if (SUCCEEDED(hresult))
    {
      ...
      {
        RJ2XCL_LOG_ERR("COM Exception in Invoke: 0x%08X", hresult);
...
void COMObjectMap::DispatchResponse(RJ2XCLBuffers::CallResponse &response, const LPDISPATCH dispatch_pointer) {
...
void COMObjectMap::DispatchToVariable(RJ2XCLBuffers::Variable *variable, LPDISPATCH dispatch_pointer, bool enums) {
  ...
  // Verify interface name length before proceeding with mapping.
  ...
  for (auto function : function_list) {
    ...
    if (function.call_type_ == MemberFunction::CallType::PropertyGet) {
      element->set_call_type(RJ2XCLBuffers::CallType::get);
    }
    else if (function.call_type_ == MemberFunction::CallType::PropertyPut) {
      element->set_call_type(RJ2XCLBuffers::CallType::put);
    }
    else {
      // this is default, so can skip
      // element->set_call_type(RJ2XCLBuffers::CallType::method);
    }

    for (auto argument : function.arguments_) {

      // Note: metadata like default values could be expanded here.
  ...
  if (SUCCEEDED(hresult)) {
    ...
    if (SUCCEEDED(hresult) && (name == match_name))
    {
      ...
      if (SUCCEEDED(hresult)) {
        ...
        default:
          RJ2XCL_LOG_WARN("Unexpected type kind: %d", type_kind);
          break;
        }
                                ...
                                if (SUCCEEDED(type_info_2->GetRefTypeInfo(handle, &type_info_3)))
                                {
                                    ...
                                    if (SUCCEEDED(hresult))
                                    {
                                        *coclass_ref = type_info_2;
                                        ...
                                        RJ2XCL_LOG_DEBUG("RC on coClass addRef: %d", refcount);
