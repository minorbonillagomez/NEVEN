...
namespace gdi_graphics_device {
    ...
    , bitmap_(0)
  {
    ...
    // GDI+ initialization required before device creation
    ...
    GetTempFileNameA(temp_path, "RJ2XCL_spreadsheet_device_", 0, temp_file);

    // set local
    temp_file_path_ = temp_file;

    // Temp file path for graphics output
  ...
  void Device::UpdateSize() {
    ...
    graphics_update->set_command(RJ2XCLBuffers::GraphicsUpdateCommand::query_size);
    graphics_update->set_name(name());
    Callback(call, response);

    auto result = response.result();

    if (result.value_case() == RJ2XCLBuffers::Variable::ValueCase::kGraphics) {
