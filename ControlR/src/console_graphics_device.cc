...
namespace ConsoleGraphicsDevice {
  ...
  __inline void RColorToVariableColor(RJ2XCLBuffers::Color *target, uint32_t src) {
  ...
  void SetMessageContext(RJ2XCLBuffers::GraphicsContext *target, const pGEcontext gc) {
  ...
  void DrawPath(double *x, double *y,
    ...
    std::cerr << "ENOTIMPL: draw_path" << std::endl; // DrawPath not supported in console device
  }
  ...
  double GetStringWidth(const char *str, const pGEcontext gc, pDevDesc dd) {
    ...
    if (success) {
      if (response.operation_case() == RJ2XCLBuffers::CallResponse::OperationCase::kConsole) {
  ...
  SEXP CreateConsoleDevice(const std::string &background, double width, double height, double pointsize, const std::string &type, void * pointer) {
    ...
    CHILD_LOG("init device (%s): %d, %d", normalized_type->c_str(), (int)dd->right, (int)dd->bottom);
    dd->deviceSpecific = normalized_type;

    std::string name = "RJ2XCL Console (";
