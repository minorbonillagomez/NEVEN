...
void SetNames(SEXP variable, const std::vector<std::string> &names) {
  ...
  for (int i = 0; i < len; i++) {
    SET_STRING_ELT(names_sexp, i, Rf_mkCharCE(names[i].c_str(), CE_UTF8));
  }
  Rf_setAttrib(variable, R_NamesSymbol, names_sexp);
}

SEXP VariableToSEXP(const RJ2XCLBuffers::Variable &var) {
  ...
  case RJ2XCLBuffers::Variable::ValueCase::kRef:
  {
    ...
    // Error case: return partially constructed object (non-fatal)
  ...
  case RJ2XCLBuffers::Variable::ValueCase::kArr:
  {
    ...
    // NOTE: int and logical could potentially be merged, and coercion
    // functions (asReal &c) could simplify this. Left as-is for clarity.
    ...
    if (type_flags & MessageUtilities::TypeFlags::numeric) {
      if (type_flags & MessageUtilities::TypeFlags::integer) {
        ...
        for (int i = 0; i < count; i++) {
          ...
          if((value_case == RJ2XCLBuffers::Variable::ValueCase::kNil) || (value_case == RJ2XCLBuffers::Variable::ValueCase::kMissing)) p[i] = NA_INTEGER;
        ...
        for (int i = 0; i < count; i++) {
          auto value_case = arr.data(i).value_case();
          if ((value_case == RJ2XCLBuffers::Variable::ValueCase::kNil) || (value_case == RJ2XCLBuffers::Variable::ValueCase::kMissing)) p[i] = NA_REAL;
          else if( value_case == RJ2XCLBuffers::Variable::ValueCase::kInteger) p[i] = arr.data(i).integer();
      ...
      for (int i = 0; i < count; i++) {
        auto value_case = arr.data(i).value_case();
        if ((value_case == RJ2XCLBuffers::Variable::ValueCase::kNil) || (value_case == RJ2XCLBuffers::Variable::ValueCase::kMissing)) p[i] = NA_LOGICAL;
      ...
      for (int i = 0; i < count; i++) {
        auto value_case = arr.data(i).value_case();
        if ((value_case == RJ2XCLBuffers::Variable::ValueCase::kNil) || (value_case == RJ2XCLBuffers::Variable::ValueCase::kMissing)) {
          SET_STRING_ELT(list, i, NA_STRING);
        }
        else {
          SET_STRING_ELT(list, i, Rf_mkCharCE(arr.data(i).str().c_str(), CE_UTF8));
        }
      }
    }
    else {
  ...
  case RJ2XCLBuffers::Variable::ValueCase::kComPointer:
  {
    ...
    for (int i = 0; i < len; i++) {
      ...
      case RJ2XCLBuffers::CallType::get:
        call_type = "get";
        function_name = call_type;
        function_name += "_";
        break;
      case RJ2XCLBuffers::CallType::put:
        call_type = "put";
        function_name = call_type;
        function_name += "_";
        break;
      ...
      for (int j = 0; j < arguments_len; j++) {
        SET_STRING_ELT(r_arguments_list, j, Rf_mkCharCE(com_function.arguments(j).name().c_str(), CE_UTF8));
      }
  ...
  case RJ2XCLBuffers::Variable::ValueCase::kErr:
    if (var.err().type() == RJ2XCLBuffers::ErrorType::NA) return Rf_ScalarInteger(NA_INTEGER);
...
SEXP ResolveCacheReference(uint32_t reference) {
  ...
  int err = 0;
  SEXP env = R_tryEvalSilent(Rf_lang2(Rf_install("get"), Rf_mkString("NEVEN")), R_GlobalEnv, &err);
...
SEXP RCallSEXP(const RJ2XCLBuffers::CompositeFunctionCall &fc, bool wait, int &err) {
  ...
  if (flags == 1) {
    ...
    for (int i = 0; i < len; i++) {
      ...
      if (argument.value_case() != RJ2XCLBuffers::Variable::ValueCase::kMissing) {
...
__inline bool HandleSimpleTypes(SEXP sexp, int len, int rtype, RJ2XCLBuffers::Array *arr, RJ2XCLBuffers::Variable *var) { // , const std::vector<std::string> &levels = {}) {
  ...
  if (Rf_isLogical(sexp) || rtype == LGLSXP)
  {
    for (int i = 0; i < len; i++) {
      ...
      if( NA_LOGICAL == lgl ){
        ptr->mutable_err()->set_type(RJ2XCLBuffers::ErrorType::NA);
      }
      else {
    ...
    if (levels && Rf_isString(levels)) {
      ...
      for (int level = 0; level < level_count; level++) {
        ...
        level_strings.push_back(R_CHAR(Rf_asChar(strsxp)));
      }
    }
  ...
  else if (isString(sexp) || rtype == STRSXP)
  {
    for (int i = 0; i < len; i++) {
      ...
      if (NA_STRING == strsxp){
        ...
        const char *sexp_string = R_CHAR(Rf_asChar(strsxp));
...
void SEXPXlReferenceToVariable(RJ2XCLBuffers::Variable *var, SEXP sexp) {
...
void SEXPToVariable(RJ2XCLBuffers::Variable *var, SEXP sexp, std::vector <SEXP> envir_list = std::vector<SEXP>()) {
  ...
  if (rtype == SYMSXP) {
    ...
    // NOTE: SYMSXP with default value F/T are symbols, not logicals.
    // These are handled by the caller via formals() inspection.
  ...
  if (Rf_isEnvironment(sexp)) {} // ...
    ...
    if (Rf_isFrame(sexp) && rtype == VECSXP) {
...

      {
        ...
        if (names && names_len && isString(names)) {
          for (int i = 0; i < names_len; i++) {
            arr->add_colnames()->assign(R_CHAR(Rf_asChar(STRING_ELT(names, i))));
          }
        }
      }

      {
        ...
        if (names && names_len && isString(names)) {
          for (int i = 0; i < names_len; i++) {
            arr->add_rownames()->assign(R_CHAR(Rf_asChar(STRING_ELT(names, i))));
          }
        }
      }
    ...
    if (HandleSimpleTypes(sexp, len, rtype, arr, var)) {
      ...
      // read external pointer logged to file only
      ...
      else if (Rf_inherits(sexp, "RJ2XCLCacheReference")) {
    ...
    if (names && names_type != 0) {
      ...
      for (int i = 0; i < len && i < nameslen && i < array_len; i++) {
        ...
        std::string str(R_CHAR(Rf_asChar(name)));
    ...
    if (dimnames && TYPEOF(dimnames) != 0) {
      if (TYPEOF(dimnames) == VECSXP) {
        ...
        if (dimnames_length > 0) {
          ...
          for (int i = 0; i < nameslen; i++)  rownames->Add(R_CHAR(Rf_asChar(STRING_ELT(name_list, i))));
        }
        if (dimnames_length > 1) {
          auto colnames = arr->mutable_colnames();
          SEXP name_list = VECTOR_ELT(dimnames, 1);
          int nameslen = Rf_length(name_list);
          for (int i = 0; i < nameslen; i++)  colnames->Add(R_CHAR(Rf_asChar(STRING_ELT(name_list, i))));
        }
        if (dimnames_length > 2) {
          // unhandled dimnames â€” silently skip
        }
      }
      else {
        // unhandled dimnames type â€” silently skip
      }
    }
...
RJ2XCLBuffers::CallResponse& ListScriptFunctions(RJ2XCLBuffers::CallResponse &response) {
  
  ParseStatus ps;

  SEXP cmds = PROTECT(Rf_allocVector(STRSXP, 1));
  SET_STRING_ELT(cmds, 0, Rf_mkChar("NEVEN$list.functions()"));
      ...
      for (auto function_entry : var.arr().data()) {
        ...
        for (auto element : function_entry.arr().data()) {
          if (element.name() == "attributes"){
            if (element.value_case() == RJ2XCLBuffers::Variable::ValueCase::kArr) {
              for (auto attribute : element.arr().data()) {
                if (attribute.name() == "description") {
                  if (attribute.value_case() == RJ2XCLBuffers::Variable::ValueCase::kArr) {
        ...
        for (auto element : function_entry.arr().data()) {
          ...
          if (!name.compare("name")) {
            ...
            if (element.value_case() == RJ2XCLBuffers::Variable::ValueCase::kInteger)
              descriptor->set_flags(element.integer());
            else if (element.value_case() == RJ2XCLBuffers::Variable::ValueCase::kReal)
...
RJ2XCLBuffers::CallResponse& ExecUserCommand(RJ2XCLBuffers::CallResponse &response, const RJ2XCLBuffers::CallResponse &call) {
...
RJ2XCLBuffers::CallResponse& RCall(RJ2XCLBuffers::CallResponse &rsp, const RJ2XCLBuffers::CallResponse &call) {
...
RJ2XCLBuffers::CallResponse& RExec(RJ2XCLBuffers::CallResponse &rsp, const RJ2XCLBuffers::CallResponse &call) {
  ...
  if (ps != PARSE_OK) {
    ...
    for (int i = 0; !err && i < Rf_length(parsed); i++) {
      SEXP cmd = VECTOR_ELT(parsed, i);
      result = R_tryEvalSilent(cmd, R_GlobalEnv, &err);
    }
    if (err) {
      // Capture R error message
      const char* errmsg = R_curErrorBuf();
      if (errmsg && errmsg[0]) {
        rsp.set_err(std::string("R: ") + errmsg);
      } else {
        rsp.set_err("R error (no message available)");
      }
    }
...
SEXP COMCallback(SEXP function_name, SEXP call_type, SEXP index, SEXP pointer_key, SEXP arguments) {
  ...
  if (isString(function_name)) string_name = (R_CHAR(Rf_asChar(STRING_ELT(function_name, 0))));
  if (isString(call_type)) string_type = (R_CHAR(Rf_asChar(STRING_ELT(call_type, 0))));
  ...
  if (!string_type.compare("get")) callback->set_type(RJ2XCLBuffers::CallType::get);
  else if (!string_type.compare("put")) callback->set_type(RJ2XCLBuffers::CallType::put);
  else if (!string_type.compare("method")) callback->set_type(RJ2XCLBuffers::CallType::method);
  ...
  if (success) {
    ...
    if (response->operation_case() == RJ2XCLBuffers::CallResponse::OperationCase::kFunctionCall) {
      ...
      function_call->set_function("RJ2XCL$install.com.pointer");
...
SEXP RCallback(SEXP command, SEXP data) {
  ...
  if (isString(command)) {
    string_command = (R_CHAR(Rf_asChar(STRING_ELT(command, 0))));
  }
  ...
  // some commands are handled here. others are sent to RJ2XCL as callbacks.
  ...
  if (!string_command.compare("spreadsheet-device")) {
    ...
    name = R_CHAR(Rf_asChar(VECTOR_ELT(data, 0)));
    background = R_CHAR(Rf_asChar(VECTOR_ELT(data, 1)));
