...
#' structures.  RJ2XCL does that to preserve type information; R vectors can't 
...
#' Create a RJ2XCL graphics device.
#'
#' \code{RJ2XCL.graphics.device} creates a graphics device that renders to 
...
#'
#' @export 
RJ2XCL.graphics.device <- function( name="RJ2XCL-default", bgcolor="white", width=400, height=400, pointsize=12, scale=Sys.getenv("RJ2XCLGraphicsScale"), cell=F ){
  ...
  if( cell ){
  	ref <- RJ2XCL$.Excel(89); # xlfCaller
	  sheetnm <- RJ2XCL$.Excel(0x4005, ref); # xlSheetNm
    ...
    .Call( "spreadsheet_device", name, bgcolor, width, height, pointsize, PACKAGE='RJ2XCLModule' );
	}
}

#' Create a console graphics device.
#'
#' \code{RJ2XCL.console.graphics.device} creates a graphics device that 
#' renders to the shell. type is either "svg" or "png".
#'
#' @export 
RJ2XCL.console.graphics.device <- function( bgcolor="white", width=500, height=350, pointsize=14, type="png"){

  # default to png, enforce
  if(type != "svg"){ type = "png"; }

  name <- paste0("RJ2XCL Console (", type, ")");
	x <- dev.list();
	if((length(x) > 0) & (name %in% names(x))){ dev.off( x[[name]]); }

  .Call( "console_device", bgcolor, width, height, pointsize, type, PACKAGE='RJ2XCLModule' );
...
js.client.progress.bar <- function( min=0, max=1, initial=min, ... ){
  ...
  invisible(.Call("progress_bar", struct, PACKAGE="RJ2XCLModule" ));
  return(handle);
}
...
#' Generic method for RJ2XCL shell progress bar
#'
#' @export 
...
#' History implementation for the RJ2XCL console
#' 
history <- function( max.show=25, reverse=FALSE, pattern, ... ){
  lines <- .Call( "history", list(), PACKAGE="RJ2XCLModule" );
...
#  .Call("Callback", command, data, PACKAGE="RJ2XCLModule");
#}
...
#        "wget", "curl", "lynx", "js"))
#      if( method == "js" ){
#        invisible(.Call("download", as.list(environment()), PACKAGE="RJ2XCLModule" ));
#      }
#      else {
...
#  options( download.file.method="js" );
  ...
  quit <- function(){ invisible(.Call("close_console", PACKAGE="RJ2XCLModule" )); }
