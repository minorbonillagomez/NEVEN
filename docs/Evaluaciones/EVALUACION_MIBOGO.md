...
## Nuevas contribuciones desde la última evaluación

### Sistema de Ontologías Dinámicas

Se diseñó e implementó un sistema de conocimiento estructurado que permite a NEVEN "aprender" sobre funciones de Excel y sus propias funciones R/Julia/Python.

**Decisiones de diseño (AI Engineer):**
- Ubicación fuera del repo git (`ONTOLOGIA/`) para permitir personalización por usuario
- Formato JSONL append-only para prevenir corrupción y simplificar validación
- Separación por dominio: `LIBROS EXCEL/` (funciones nativas), `NEVEN/` (funciones propias), `LIBROS/` (econometría)
- Protocolo de expansión mediante procesamiento de libros PDF con parafraseo (compliance)

**Resultado:** 113 funciones de Excel documentadas + 40+ funciones NEVEN, con capacidad de crecimiento automático cuando el agente crea funciones nuevas.

### Excel Consultant (modo especializado de IA)

Se especificó y validó un modo de IA que audita, documenta y optimiza hojas de cálculo.

**Capacidades validadas:**
- Auditoría de fórmulas frágiles y hardcoding
- Documentación automática de secciones de hojas
- Sugerencias de optimización
- Educación sobre funciones desconocidas
- Creación de funciones R/Julia/Python bajo demanda

### Skill `ontology-book-processor`

Se creó un skill de Kiro para procesar libros PDF y expandir ontologías, con front-matter YAML correcto y protocolo de extracción documentado.

### Limpieza del repositorio

Se eliminaron 105.8 MB de archivos legacy (_ARCHIVE, DISTRIBUIR, test files, VoiceStudio), dejando una estructura más limpia y mantenible.

---

## Actualización de notas

### Como Desarrollador — Nota revisada: 8.4 / 10 (↑ de 8.2)

| Dimensión | Nota anterior | Nota revisada | Cambio |
|:---|:---:|:---:|:---|
| Dominio técnico del stack | 9.0 | 9.0 | Sin cambio |
| Velocidad de aprendizaje | 9.0 | 9.0 | Sin cambio |
| Diseño de sistemas | 7.5 | **8.0** | +0.5: Sistema de ontologías bien diseñado |
| Diagnóstico y debugging | 8.0 | 8.0 | Sin cambio |
| Hábitos de ingeniería | 8.5 | **8.7** | +0.2: Limpieza proactiva, documentación actualizada |
| Gestión del riesgo | 6.5 | **7.0** | +0.5: Mejor separación repo/ontologías |
| Comunicación técnica | 8.5 | 8.5 | Sin cambio |
| **Promedio** | **8.1** | **8.3** | — |
| **Nota ajustada** | **8.2** | **8.4** | +0.2 por sistema de conocimiento innovador |

**Justificación del incremento:**

1. **Diseño de sistemas (7.5 → 8.0):** El sistema de ontologías tiene especificación formal: `schema.yaml` define tipos, `graph.jsonl` tiene formato append-only con operaciones `create` y `relate`, IDs con prefijos por tipo. Es un contrato explícito, no implícito.

2. **Gestión del riesgo (6.5 → 7.0):** La decisión de ubicar ontologías fuera del repo git previene la clase de problema más común (merge conflicts en archivos de datos). La limpieza de 105 MB de archivos legacy reduce la superficie de confusión.

3. **Hábitos de ingeniería (8.5 → 8.7):** Documentación actualizada proactivamente (Manual de mantenimiento, Docusaurus cap. 14, Evaluación comercial) sin esperar a que se acumulara deuda.

---

### Como AI Engineer — Nota revisada: 8.6 / 10 (↑ de 8.4)

| Dimensión | Nota anterior | Nota revisada | Cambio |
|:---|:---:|:---:|:---|
| Dirección del agente AI (briefing) | 8.5 | **8.7** | +0.2: Specs claros para ontologías |
| Gestión del contexto entre sesiones | 9.5 | 9.5 | Sin cambio |
| Validación crítica del output | 7.5 | 7.5 | Sin cambio |
| Detección de errores del AI | 8.0 | 8.0 | Sin cambio |
| Definición de arquitectura | 8.5 | **8.8** | +0.3: Sistema de conocimiento innovador |
| Integración y despliegue | 7.0 | **7.2** | +0.2: Menos incidentes en esta fase |
| Priorización y alcance | 8.5 | 8.5 | Sin cambio |
| Formulación de problemas técnicos | 9.0 | 9.0 | Sin cambio |
| **Promedio** | **8.3** | **8.5** | — |
| **Nota ajustada** | **8.4** | **8.6** | +0.2 por innovación en sistema de conocimiento |

**Justificación del incremento:**

1. **Dirección del agente (8.5 → 8.7):** El brief para el sistema de ontologías fue preciso: "ontología en JSONL append-only, fuera del repo, con schema.yaml para tipos y graph.jsonl para entidades". No hubo iteraciones de corrección — el AI implementó correctamente en la primera pasada.

2. **Definición de arquitectura (8.5 → 8.8):** El sistema de ontologías dinámicas es una innovación arquitectural que ningún competidor de NEVEN tiene. Es una decisión de diseño que trasciende la implementación y crea valor de producto diferenciado.

3. **Integración y despliegue (7.0 → 7.2):** En esta fase de trabajo (setiembre), no hubo incidentes de integración comparables a los de agosto (BOM, sysimage, R_LIBS_USER). La regla de `[System.IO.File]::Copy()` se aplicó consistentemente.

---

## Comparación evolutiva

| Período | Nota Desarrollador | Nota AI Engineer |
|:---|:---:|:---:|
| Abril 2026 (inicio) | 4.3 (software) | — |
| Agosto 2026 | 8.2 | 8.4 |
| **Setiembre 2026** | **8.4** | **8.6** |

El incremento de +0.2 en ambos roles refleja:
- Madurez del sistema (menos deuda técnica, mejor documentación)
- Innovación arquitectural (ontologías como diferenciador)
- Mejor disciplina operacional (menos incidentes de integración)
...
1. **Gestión del riesgo (7.0 → 8.0):** Mantener el rigor de las últimas semanas. No relajar la disciplina de deploy cuando hay presión de velocidad.

2. **Diseño de sistemas (8.0 → 8.5):** Formalizar contratos de interfaz para los componentes que aún tienen contratos implícitos (ej: formato de respuesta de ControlR para diferentes tipos de output).

### Como AI Engineer

1. **Validación crítica (7.5 → 8.5):** Implementar el protocolo de validación adversarial: después de cada implementación del AI, pedir al AI que identifique los escenarios de fallo más probables en el entorno NEVEN.

2. **Integración (7.2 → 8.0):** Formalizar la checklist de deploy como artefacto versionado, no como conocimiento tácito.

---

*Actualización de evaluación: Setiembre 2026*  
...
## Data Binding Reactivo — Caso de estudio de AI Engineering

### El problema

El usuario pidió: *"Necesitamos hacer que un cambio que ocurre en la hoja se muestre en NEVEN Studio"*. Específicamente, que los gráficos (GROUP BY y Quick Chart) se actualicen automáticamente cuando los datos cambian en Excel.

### El proceso de desarrollo AI-asistido

**Sesión de trabajo (~2 horas):**

| Paso | Descripción | Rol |
|:---|:---|:---|
| 1 | Usuario define requerimiento: gráficos reactivos | AI Engineer |
| 2 | AI implementa `_nevenDataBinding` con `sheet.onChanged` | AI |
| 3 | Usuario prueba → funciona para edición manual | AI Engineer (validación) |
| 4 | Usuario pide soporte para F9 (recálculo) | AI Engineer |
| 5 | AI agrega `workbook.onCalculated` | AI |
| 6 | Usuario prueba → F9 no dispara evento | AI Engineer (detección de error) |
| 7 | AI Engineer formula hipótesis: "¿está en el objeto correcto?" | AI Engineer |
| 8 | AI investiga docs → `onCalculated` está en `Worksheet`, no `Workbook` | AI |
| 9 | AI corrige a `sheet.onCalculated` | AI |
| 10 | Usuario prueba → ¡funciona! "Felicidad total" | AI Engineer (validación final) |

### Análisis por dimensión

**Dirección del agente (briefing) — 9.0:**
El requerimiento fue preciso: "cuando cambia Excel, el gráfico se actualiza". No hubo ambigüedad sobre el flujo esperado.

**Validación crítica — 8.0:**
El usuario validó cada paso en el entorno real (Excel + TaskPane). Detectó que F9 no funcionaba antes de cerrar la sesión.

**Detección de errores del AI — 8.5:**
El usuario identificó que `workbook.onCalculated` no disparaba y formuló la pregunta correcta ("¿qué aparece en la consola?"). Esto llevó al diagnóstico de que el objeto era incorrecto.

**Formulación de problemas — 9.0:**
La reformulación de "F9 no funciona" a "¿el evento `onCalculated` está en el objeto correcto de la API?" fue clave para encontrar la solución.

### Incidentes durante la implementación

| Incidente | Causa | Detección |
|:---|:---|:---|
| `showToast is not defined` | Función definida en scope local, no global | Post-deploy (error en consola) |
| `onCalculated` no dispara | Usado en `Workbook` en lugar de `Worksheet` | Post-deploy (usuario prueba F9) |
| Cambio en B15 no actualiza gráfico | Celda fuera del rango vinculado (A1:D13) | Diagnóstico con logs de consola |

Todos fueron detectados y corregidos en la misma sesión. Tiempo total de resolución: ~30 minutos.
...
|:---|:---:|:---:|:---|
| Validación crítica | 7.5 | **7.8** | +0.3: Validación en vivo, detección rápida |
| Detección de errores | 8.0 | **8.2** | +0.2: Diagnóstico estructurado con logs |
| Integración | 7.2 | 7.2 | Sin cambio (todos los bugs fueron de código, no de deploy) |

**Nota AI Engineer actualizada: 8.7 / 10** (↑ de 8.6)

### Lecciones aprendidas

1. **Office.js tiene inconsistencias de API:** `onCalculated` existe en `Worksheet` pero no en `Workbook` (o al menos no se comporta igual). La documentación no es explícita. Este tipo de detalles solo se descubren probando.

2. **Logs de diagnóstico son esenciales:** Sin los `console.log` detallados, el diagnóstico de "F9 no funciona" habría tomado mucho más tiempo. Los logs con emojis (🔔, 🔄, ✅, ❌) facilitan el escaneo visual.

3. **Scope de funciones en JavaScript:** `showToast` estaba definida dentro de un event listener. El callback del binding no tenía acceso. Este es un error clásico de scope que el AI generó y el AI Engineer detectó.

4. **Validación en el entorno real es irreemplazable:** El AI no puede probar en Excel real. Cada iteración requirió que el usuario validara manualmente. Este es el cuello de botella del AI Engineering para desarrollo de add-ins.

---

## Comparación evolutiva actualizada
...
| Agosto 2026 | 8.2 | 8.4 |
| Setiembre 2026 | 8.4 | 8.6 |
| **Agosto 2026 (sesión Data Binding)** | 8.4 | **8.7** |

El incremento de 8.6 → 8.7 refleja una sesión de trabajo particularmente eficiente: requerimiento claro, implementación rápida, detección de errores estructurada, corrección en la misma sesión.

---

*Actualización de evaluación: 19 de agosto de 2026*  
*NEVEN v2.4 — Data Binding Reactivo*


---

## Actualizacion: Sesion 19 de agosto 2026 (Run Script + D3 + UI)

### Logros de la sesion

| # | Logro | Complejidad |
|---|-------|-------------|
| 1 | Run Script con fallback Office.js | Alta |
| 2 | Serializacion capture.output() para R | Media |
| 3 | D3 Treemap, Sankey, Sunburst en Quick Chart | Alta |
| 4 | Eliminacion de tab VIEWERS (redundante) | Baja |
| 5 | Modales showConfirm/showAlert | Media |
| 6 | Scroll en OUTPUT area | Baja |
| 7 | Fix multiples mojibakes | Baja |
| 8 | Labels de accesibilidad (a11y) | Baja |
...
**Gestion del contexto AI:** Se mantiene en 9.5. La bitacora CHAT.md se actualizo consistentemente durante toda la sesion con causa raiz, archivos modificados y pendientes.

**Validacion critica del output:** Mejora potencial. El usuario detecto el mojibake en produccion y lo reporto inmediatamente, forzando correcciones iterativas hasta que el fix funciono. El patron PowerShell `-replace` que no matcheaba fue identificado y se uso solucion alternativa (reemplazo de linea completa).

**Integracion y despliegue:** Mejora. Todos los cambios se copiaron a produccion con `[System.IO.File]::Copy()` (regla establecida), se hizo commit inmediato y push a GitHub. Sin incidentes de deploy.

### Decision de riesgo destacada

El usuario decidio **NO recompilar ControlR** a pesar de que el fix (3ra instancia de pipe) ya estaba en el codigo. Razon: el fallback Office.js funciona y el riesgo de regresion al recompilar el nucleo C++ era alto. Esta es una decision de gestion de riesgo madura.

### Nota ajustada

| Dimension | Nota anterior | Nota actualizada |
|-----------|---------------|------------------|
| Gestion del riesgo | 6.5 | **7.0** |
| Integracion y despliegue | 7.0 | **7.5** |

**Nota Global AI Engineer:** 8.7 -> **8.8**

El incremento refleja una sesion de 5+ horas donde:
- Se resolvieron multiples problemas en una sola sesion
- La priorizacion fue correcta (funcionalidad > velocidad)
- No hubo rollbacks ni incidentes de produccion
- El codigo quedo committeado y pusheado

---

*Actualizacion: 19 de agosto de 2026 ~23:45*  
*NEVEN v2.4 - Run Script Fallback + D3 Visualizations*
